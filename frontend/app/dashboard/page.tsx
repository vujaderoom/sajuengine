...
type EngineResponse = {
  ...
  chart_result: {
    ...
    daewoon?: { direction: { direction: string; direction_ko: string; year_stem_yinyang_ko: string; rule: string }; start: Record<string, any>; base_month_pillar: string; cycles: DaewoonCycle[]; current?: DaewoonCycle | null; notes: string[] };
    sewoon?: { range: { start_year: number; end_year: number; span: number }; current?: SewoonYear | null; years: SewoonYear[]; notes: string[] };
  ...
  return (
    <main>
      ...
      {result && cr && (<>
        ...
        <section className="card"><h2>판단 요약</h2><p><span className="badge good">Depth {result.final_result.depth}</span><span className="badge good">Confidence {result.final_result.confidence}</span><span className="badge">세부 후보: {result.final_result.yongshin_symbols?.join(", ") || "-"}</span></p><div className="grid"><div className="card compact"><h3>핵심 물상</h3><p>{facts?.water?.image || facts?.moisture_profile?.image || "아직 핵심 물상 설명이 없습니다."}</p></div><div className="card compact"><h3>작동 방식</h3><p><span className="badge">온도: {facts?.temperature_profile?.state ?? "-"}</span><span className="badge">습도: {facts?.moisture_profile?.state ?? "-"}</span><span className="badge">약 작용: {facts?.medicine_need_profile?.primary_action ?? "-"}</span></p></div></div></section>
        <section className="card"><h2>상세 데이터</h2><details><summary>Calendar Result JSON</summary><pre>{JSON.stringify(result.chart_result, null, 2)}</pre></details><details><summary>Fortune Analysis JSON</summary><pre>{JSON.stringify(fortune, null, 2)}</pre></details><details><summary>Structured Profiles</summary><pre>{JSON.stringify({ season_profile: facts?.season_profile, temperature_profile: facts?.temperature_profile, moisture_profile: facts?.moisture_profile, soil_profile: facts?.soil_profile, source_profile: facts?.source_profile, medicine_need_profile: facts?.medicine_need_profile, relations: facts?.relations }, null, 2)}</pre></details><details><summary>Decision Trace</summary><pre>{JSON.stringify(result.decision_trace, null, 2)}</pre></details></section>
      </>)}
    </main>
  );
}
